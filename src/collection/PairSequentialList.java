...
public class PairSequentialList<S, T, U> extends AbstractSequentialList<U> {
    ...
    public PairSequentialList(List<S> left, List<T> right, BiFunction<S, T, U> map) {
        this.left = left;
        this.right = right;
        this.map = map;
    }
    ...
    public ListIterator<U> listIterator(int index) {
        return  new PairListIterator<>(left.listIterator(index), right.listIterator(index), map, index);
    }
